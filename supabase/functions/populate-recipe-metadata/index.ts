...
async function processHotdogsInBackground(
  supabase: any,
  LOVABLE_API_KEY: string,
  hotdogs: any[]
) {
  console.log(`Starting background processing of ${hotdogs?.length || 0} hotdogs`);
...
serve(async (req) => {
  ...
  try {
    ...
    const totalCount = hotdogs?.length || 0;
    console.log(`Starting background processing for ${totalCount} hotdogs`);

    // Start background processing WITHOUT awaiting - this is the key change
    processHotdogsInBackground(supabase, LOVABLE_API_KEY, hotdogs || []);
    ...
    return new Response(
      JSON.stringify({ 
        ...
        total: totalCount,
        note: "Check edge function logs for progress. This will take 2-3 minutes. Refresh the page after waiting to see updated data."
      }),
      { 
